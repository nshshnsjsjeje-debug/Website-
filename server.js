...
let pgPool=null;
try{ if(process.env.DATABASE_URL){ const {Pool}=require('pg'); pgPool=new Pool({connectionString:process.env.DATABASE_URL,ssl:process.env.DATABASE_URL.includes('localhost')?false:{rejectUnauthorized:false}}); } }catch(e){ console.error('Postgres unavailable, using local JSON storage:',e.message); }
...
const save=()=>{
  ...
  if(pgPool){
    saveInFlight=pgPool.query(`INSERT INTO falcon_state(id,data,updated_at) VALUES(1,$1::jsonb,NOW()) ON CONFLICT(id) DO UPDATE SET data=EXCLUDED.data,updated_at=NOW()`,[JSON.stringify(db)]).catch(e=>console.error('Postgres save error:',e.message));
  }
  return saveInFlight;
};
...
app.use(express.json({limit:'3mb'}));app.use(express.urlencoded({extended:true,limit:'3mb'}));
...
if(pgPool){
 persistenceReady=(async()=>{
   await pgPool.query(`CREATE TABLE IF NOT EXISTS falcon_state (id INTEGER PRIMARY KEY, data JSONB NOT NULL, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`);
   const row=(await pgPool.query('SELECT data FROM falcon_state WHERE id=1')).rows[0];
   if(row && row.data){ db=typeof row.data==='string'?JSON.parse(row.data):row.data; normalizeDb(); }
   ensureOwner();
   await pgPool.query(`INSERT INTO falcon_state(id,data,updated_at) VALUES(1,$1::jsonb,NOW()) ON CONFLICT(id) DO NOTHING`,[JSON.stringify(db)]);
 })().catch(e=>{console.error('Persistent database init failed:',e);});
}else{ ensureOwner(); }
app.use(async(req,res,next)=>{ try{ await persistenceReady; next(); }catch(e){ next(e); } });
...
app.get('/admins.html',staffPage('admin.html'));
app.get('/received.html',staffPage('received.html'));
app.get('/support-inbox.html',staffPage('support-inbox.html'));
...
app.post('/api/owner/products',(req,res)=>{
 ...
 const name=String(req.body.name||'').trim(),icon=String(req.body.icon||'🛍️').trim(),desc=String(req.body.desc||'').trim();
 ...
 db.products[id]={id,name,icon,desc,price:salePrice,originalPrice,discountPercent,stockQty,stockEnabled,hiddenByCode};
 save();
...
app.post('/api/settings',(req,res)=>{
 const u=getUserBySession(req); if(!u)return res.status(401).json({error:'سجل دخولك أولاً'});
 const username=String(req.body.username||'').trim(),oldPassword=String(req.body.oldPassword||''),newPassword=String(req.body.newPassword||''),avatar=String(req.body.avatar||'').trim(),avatarData=String(req.body.avatarData||'').trim();
 ...
 if(avatarData){ if(!/^data:image\/(png|jpe?g|webp|gif);base64,/i.test(avatarData))return res.status(400).json({error:'ملف الصورة غير صالح.'}); if(avatarData.length>2200000)return res.status(400).json({error:'الصورة كبيرة جدًا. اختر صورة أقل من حوالي 1.5MB.'}); u.avatar=avatarData; } else { u.avatar=avatar.slice(0,500); }
...
app.post('/api/owner/gift-user',(req,res)=>{
 ...
 const key=String(req.body.userId||req.body.username||'').trim().toLowerCase();
 const u=Object.values(db.users).find(x=>x && (String(x.id)===key || String(x.username||'').toLowerCase()===key));
 if(!u)return res.status(404).json({error:'العضو غير موجود'});
 u.coins=Number(u.coins||0)+amount;
 notifyUser(u.id,'✈️ تم إرسال عملات لك',`تمت إضافة ${amount.toLocaleString('ar-EG')} عملة فالكون إلى رصيدك بواسطة الإدارة.`,'gift','الإدارة');
 save();
 res.json({ok:true,user:{id:u.id,username:u.username,coins:u.coins},amount});
});
