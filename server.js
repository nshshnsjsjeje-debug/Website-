...
app.get('/api/owner',(req,res)=>{if(!staff(req))return res.status(403).json({error:'لصاحب الموقع فقط'});res.json({purchases:db.orders,supportMessages:db.messages,tickets:db.tickets,refs:db.refs,auctions:db.auctions,offers:db.offers,unlockCodes:Object.values(db.unlockCodes),users:Object.values(db.users).map(u=>({id:u.id,user:u.name,email:u.email,coins:u.coins}))});});
...
app.post('/api/gifts/:id/claim',(req,res)=>{
 ...
 db.notifications.push({id:crypto.randomUUID(),recipientId:u.id,type:'gift',title:'✅ تم استلام العملات',text:`تمت إضافة ${Number(gift.amount||0).toLocaleString('ar-EG')} عملة إلى رصيدك بنجاح.`,createdAt:new Date().toISOString(),readBy:[]});
 save();
