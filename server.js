...
app.get('/api/notifications',(req,res)=>{
 ...
 const unread=db.notifications.filter(n=>!(n.readBy||[]).includes(u.id)).slice(-10);
...
app.post('/api/owner/gift-all',(req,res)=>{
 ...
 let count=0;for(const u of Object.values(db.users)){u.coins=Number(u.coins||0)+amount;count++;}
 db.notifications.push({id:crypto.randomUUID(),title:'🎁 هدية من إدارة Falcon on Top',text:`تمت إضافة ${amount.toLocaleString('ar-EG')} عملة فالكون إلى رصيدك.`,createdAt:new Date().toISOString(),readBy:[]});
 if(db.notifications.length>100)db.notifications=db.notifications.slice(-100);
