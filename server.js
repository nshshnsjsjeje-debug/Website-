...
app.post('/api/cart/checkout',(req,res)=>{const u=getUserBySession(req);if(!u)return res.status(401).json({error:'سجل دخولك أولاً'});const note=String(req.body.note||'').trim();const couponCode=String(req.body.couponCode||'').trim().toUpperCase();if(!note)return res.status(400).json({error:'اكتب اسمك في ديسكورد أو البيانات المطلوبة'});const c=userCart(u);if(!c.length)return res.status(400).json({error:'السلة فارغة'});let total=c.reduce((a,x)=>a+Number(x.price)*Number(x.qty),0);let discount=0;let coupon=null;if(couponCode){coupon=db.coupons[couponCode];if(!coupon)return res.status(400).json({error:'الكوبون غير موجود'});if(coupon.usedBy.includes(u.id))return res.status(400).json({error:'استخدمت هذا الكوبون من قبل'});if(coupon.maxUses&&coupon.usedBy.length>=coupon.maxUses)return res.status(400).json({error:'انتهت استخدامات الكوبون'});discount=Math.floor(total*coupon.percent/100);total-=discount;}if(u.coins<total)return res.status(400).json({code:'NO_COINS',error:'ليس لديك عملات فالكون كافية'});for(const item of c){
   for(let i=0;i<Number(item.qty||1);i++){
     ...
     const paid=Math.max(0,Math.floor(Number(item.price)));
     db.orders.unshift({id:crypto.randomUUID(),userId:u.id,user:u.name,product:item.name,productId:item.product||'',price:paid,amountPaid:paid,originalPrice,discountPercent:itemDiscount,note,date:new Date().toISOString(),status:'سيتم التسليم',refunded:false});
   }
 }
 u.coins-=total;if(coupon)coupon.usedBy.push(u.id);db.carts[u.id]=[];
 notifyUser(u.id,'تم الطلب 🛒','تم إضافة طلبك إلى مشترياتي وحالته: سيتم التسليم.','order','Falcon');
 notifyStaff('🛒 طلب شراء جديد',`العضو ${u.name} اشترى من السلة. الإجمالي ${total.toLocaleString('ar-EG')} عملة. افتح مركز الإدارة للتسليم أو الإلغاء.`,'staffOrder');
 db.notifications.slice(-staffUserIds().length).forEach(n=>{if(n.type==='staffOrder' && !n.orderId)n.orderId=db.orders[0]?.id;});
 save();res.json({ok:true,coins:u.coins,cart:[],total,discount});});
...
app.post('/api/order',(req,res)=>{
 ...
 u.coins-=price;
 const originalPrice=product?Number(product.originalPrice||product.price):price;
 const discountPercent=product?Number(product.discountPercent||0):0;
 db.orders.unshift({id:crypto.randomUUID(),userId:u.id,user:u.name,email:u.email,product:name,productId:rawProduct,price,amountPaid:price,originalPrice,discountPercent,note,date:new Date().toISOString(),status:'سيتم التسليم',refunded:false});
