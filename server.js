...
const owner=req=>!!(req.session.user&&((req.session.user.username||'').toLowerCase()==='3zazel' || (process.env.OWNER_USERNAME&&req.session.user.username===process.env.OWNER_USERNAME)));
app.use(express.json());app.use(express.urlencoded({extended:true}));
app.use(session({secret:process.env.SESSION_SECRET||'falcon-secret-change-me',resave:false,saveUninitialized:false,cookie:{httpOnly:true,sameSite:'lax',secure:process.env.NODE_ENV==='production',maxAge:1000*60*60*24*7}}));
...
app.post('/api/register',(req,res)=>{
 try{
   ...
   if(db.users[username.toLowerCase()]) return res.status(400).json({error:'اسم المستخدم مستخدم بالفعل.'});
   if(username.toLowerCase()==='3zazel' || (process.env.OWNER_USERNAME && username.toLowerCase()===process.env.OWNER_USERNAME.toLowerCase()))
...
app.post('/api/login',(req,res)=>{
 try{
   ...
   save();
   res.json({ok:true,user:u,message:'تم تسجيل الدخول'});
 }catch(e){
   console.error(e);
...
app.get('/api/me',(req,res)=>{
 ...
 const key=Object.keys(db.users).find(k=>db.users[k].id===req.session.user.id);
 const u=key?db.users[key]:null;
 if(!u)return res.json({loggedIn:false});
 const changed=giveDaily(u);
 if(changed)save();
 res.json({loggedIn:true,user:u,owner:owner(req)});
});
