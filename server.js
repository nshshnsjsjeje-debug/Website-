...
if(!Array.isArray(db.staffNotifications)) db.staffNotifications=[];
...
const notifyStaff=(title,text,orderId='',type='order')=>{db.staffNotifications.unshift({id:crypto.randomUUID(),title,text,orderId,type,createdAt:new Date().toISOString(),readBy:[]});if(db.staffNotifications.length>300)db.staffNotifications=db.staffNotifications.slice(0,300);};
...
function catalog(){ return Object.keys(DEFAULT_PRODUCTS).map(k=>db.products[k] ? {...DEFAULT_PRODUCTS[k],...db.products[k]} : DEFAULT_PRODUCTS[k]).concat(Object.keys(db.products).filter(k=>!DEFAULT_PRODUCTS[k]).map(k=>db.products[k])); }
function productPricing(p){ const base=Number(p.price||0); const percent=Math.max(0,Math.min(100,Number(p.discountPercent||0))); const final=percent?Math.max(0,Math.floor(base*(100-percent)/100)):base; return {...p,originalPrice:base,discountPercent:percent,finalPrice:final,stockLabel:p.stock?'متوفر':'Un stock'}; }
function productById(id){ const p=catalog().find(p=>p.id===id); return p?productPricing(p):null; }
...
function finalizeExpiredAuctions(){let changed=false;for(const a of db.auctions.filter(x=>x.status==='active'&&x.endsAt&&new Date(x.endsAt).getTime()<=Date.now())){const winner=(a.bids||[]).slice().sort((x,y)=>y.amount-x.amount)[0]||null;a.status='closed';a.closedAt=new Date().toISOString();a.winner=winner;if(winner){const u=Object.values(db.users).find(x=>x.id===winner.userId);if(u)notifyUser(u.id,'🥇 نتيجة المزاد',`انتهى مزاد ${a.title} وفزت بأعلى مزايدة ${Number(winner.amount).toLocaleString('ar-EG')} عملة.`,'auction','Falcon');}notifyStaff('🏆 انتهى مزاد',`${a.title} انتهى. ${winner?`الفائز: ${winner.user} — ${winner.amount.toLocaleString('ar-EG')} 🪙`:'بدون مزايدات'}`,'','auction');changed=true;}if(changed)save();}
function activeAuctions(){ finalizeExpiredAuctions(); return db.auctions.filter(a=>a.status==='active' && (!a.endsAt || new Date(a.endsAt).getTime()>Date.now())); }
...
app.post('/api/auctions/:id/bid',(req,res)=>{
 ...
 a.bids=Array.isArray(a.bids)?a.bids:[];a.bids.push({id:crypto.randomUUID(),userId:u.id,user:u.name,amount,date:new Date().toISOString()});notifyStaff('🔥 مزايدة جديدة',`${u.name} زاد على مزاد ${a.title} إلى ${amount.toLocaleString('ar-EG')} 🪙`,'','auction');
...
app.post('/api/owner/products',(req,res)=>{if(!owner(req))return res.status(403).json({error:'لصاحب الموقع فقط'});const id=String(req.body.id||'').trim()||'p_'+crypto.randomUUID().slice(0,8),name=String(req.body.name||'').trim(),icon=String(req.body.icon||'🛍️').trim(),desc=String(req.body.desc||'').trim(),price=Math.floor(Number(req.body.price)),stock=!!req.body.stock,hiddenByCode=!!req.body.hiddenByCode,discountPercent=Math.max(0,Math.min(100,Math.floor(Number(req.body.discountPercent||0))));if(!name||!Number.isFinite(price)||price<0)return res.status(400).json({error:'بيانات المنتج غير صحيحة'});db.products[id]={id,name,icon,desc,price,stock,hiddenByCode,discountPercent};save();res.json({ok:true,product:productById(id)});});
app.post('/api/staff/products/:id/discount',(req,res)=>{if(!staff(req))return res.status(403).json({error:'للإدارة أو صاحب الموقع فقط'});const p=db.products[req.params.id]||DEFAULT_PRODUCTS[req.params.id];if(!p)return res.status(404).json({error:'المنتج غير موجود'});if(!db.products[req.params.id])db.products[req.params.id]={...p};const percent=Math.max(0,Math.min(100,Math.floor(Number(req.body.discountPercent||0))));p.discountPercent=percent;save();res.json({ok:true,product:productById(req.params.id)});});
...
app.post('/api/cart/checkout',(req,res)=>{
 const u=getUserBySession(req);if(!u)return res.status(401).json({error:'سجل دخولك أولاً'});
 const note=String(req.body.note||'').trim(),couponCode=String(req.body.couponCode||'').trim().toUpperCase();if(!note)return res.status(400).json({error:'اكتب اسمك في ديسكورد أو البيانات المطلوبة'});
 const c=userCart(u);if(!c.length)return res.status(400).json({error:'السلة فارغة'});
 let total=0;const items=[];
 for(const item of c){const p=productById(item.product);if(!p||!p.stock)return res.status(400).json({error:`المنتج ${item.name||item.product} غير متوفر حالياً`});const qty=Math.max(1,Number(item.qty||1));const price=Number(p.finalPrice);total+=price*qty;items.push({product:p.id,name:p.name,price,qty});}
 ...
 if(u.coins<total)return res.status(400).json({code:'NO_COINS',error:'ليس لديك عملات فالكون كافية'});
 const created=[];for(const item of items){for(let i=0;i<item.qty;i++){const order={id:crypto.randomUUID(),userId:u.id,user:u.name,product:item.name,productId:item.product,price:item.price,note,date:new Date().toISOString(),status:'سيتم التسليم'};db.orders.unshift(order);created.push(order);notifyStaff('🛒 طلب شراء جديد',`${u.name} اشترى ${item.name} — المبلغ ${item.price.toLocaleString('ar-EG')} 🪙`,order.id,'order');}}
 u.coins-=total;if(coupon)coupon.usedBy.push(u.id);db.carts[u.id]=[];notifyUser(u.id,'تم تسجيل مشترياتك 🛒',`تم تسجيل ${created.length} طلب/طلبات وحالتها: سيتم التسليم.`,'order','Falcon');save();res.json({ok:true,coins:u.coins,cart:[],total,discount,orders:created});
});
app.get('/api/purchases',(req,res)=>{const u=getUserBySession(req);if(!u)return res.status(401).json({error:'سجل دخولك أولاً'});res.json({orders:db.orders.filter(o=>o.userId===u.id)});});
...
app.post('/api/owner/order-status',(req,res)=>{
 ...
 const o=db.orders.find(x=>x.id===String(req.body.id||''));const status=String(req.body.status||'').trim();
 if(!o||!['قيد المراجعة','سيتم التسليم','تم التسليم','ملغي'].includes(status))return res.status(400).json({error:'بيانات الحالة غير صحيحة'});
 if(o.status==='تم التسليم'||o.status==='ملغي')return res.status(400).json({error:'تم إغلاق هذا الطلب بالفعل'});
 if(status==='ملغي'){const refund=Number(o.price||0);const u=Object.values(db.users).find(x=>x.id===o.userId);if(u){u.coins=Number(u.coins||0)+refund;notifyUser(u.id,'تم إلغاء طلبك ❌',`تم إلغاء طلب ${o.product} وإرجاع ${refund.toLocaleString('ar-EG')} عملة فالكون إلى رصيدك.`,'order','إدارة Falcon');}}
 if(status==='تم التسليم')notifyUser(o.userId,'تم تسليم طلبك ✅',`تم تسليم ${o.product} بنجاح.`,'order','إدارة Falcon');
 if(status==='قيد المراجعة')notifyUser(o.userId,'طلبك قيد المراجعة ⏳',`طلب ${o.product} الآن قيد المراجعة.`,'order','إدارة Falcon');
 o.status=status;o.updatedAt=new Date().toISOString();save();res.json({ok:true,order:o});
});
app.get('/api/staff-notifications',(req,res)=>{if(!staff(req))return res.status(403).json({error:'للإدارة أو صاحب الموقع فقط'});const rows=db.staffNotifications.map(n=>({...n,read:(n.readBy||[]).includes(getUserBySession(req).id)}));res.json({notifications:rows,unread:rows.filter(n=>!n.read).length});});
app.post('/api/staff-notifications/read-all',(req,res)=>{if(!staff(req))return res.status(403).json({error:'للإدارة أو صاحب الموقع فقط'});const u=getUserBySession(req);for(const n of db.staffNotifications){n.readBy=Array.isArray(n.readBy)?n.readBy:[];if(!n.readBy.includes(u.id))n.readBy.push(u.id);}save();res.json({ok:true});});
...
app.post('/api/settings',(req,res)=>{
 const u=getUserBySession(req); if(!u)return res.status(401).json({error:'سجل دخولك أولاً'});
 const username=String(req.body.username||'').trim(),oldPassword=String(req.body.oldPassword||''),newPassword=String(req.body.newPassword||''),avatarUrl=String(req.body.avatarUrl||'').trim();
 ...
 if(avatarUrl && !/^https?:\/\//i.test(avatarUrl))return res.status(400).json({error:'رابط الصورة يجب أن يبدأ بـ http:// أو https://'});
 ...
 if(avatarUrl)u.avatarUrl=avatarUrl; else if(req.body.avatarUrl==='')u.avatarUrl='';
...
app.post('/api/reviews',(req,res)=>{
 ...
 const r={id:crypto.randomUUID(),product,userId:u.id,user:u.name,avatarUrl:u.avatarUrl||'',rating,text,date:new Date().toISOString()};
...
app.post('/api/order',(req,res)=>{
 ...
 let price=0,name='',stock=false,productId=rawProduct;
 if(rawProduct.startsWith('offer:')){
   ...
   price=Number(o.price);name=o.title;stock=true;productId=rawProduct;
 }else{
   const p=productById(rawProduct);
   if(!p)return res.status(400).json({error:'منتج غير صالح'});
   price=Number(p.finalPrice);name=p.name;stock=!!p.stock;
 }
 ...
 u.coins-=price;
 const order={id:crypto.randomUUID(),userId:u.id,user:u.name,email:u.email,product:name,productId,price,note,date:new Date().toISOString(),status:'سيتم التسليم'};
 db.orders.unshift(order);
 notifyUser(u.id,'تم تسجيل طلبك 🛒',`تم تسجيل طلب ${name} وسيتم التسليم.`, 'order','Falcon');
 notifyStaff('🛒 طلب شراء جديد',`${u.name} اشترى ${name} — المبلغ ${price.toLocaleString('ar-EG')} 🪙`,order.id,'order');
 save();
 res.json({ok:true,coins:u.coins,order,message:'تم تسجيل طلبك وسيتم التسليم.'});
});
