...
const DEFAULT_PRODUCTS={
 customCar:{id:'customCar',name:'عربية مخصصة',icon:'🚘',desc:'عربية بمواصفاتك أنت فقط',price:5000,stock:true,stockQty:10,stockEnabled:true},
 gangAlley:{id:'gangAlley',name:'حارة خاصة بعصابتك',icon:'🏴',desc:'حارة خاصة مرتبطة بعصابتك',price:8000,stock:true,stockQty:10,stockEnabled:true},
 customSkin:{id:'customSkin',name:'سكن مخصص لك فقط',icon:'🧍',desc:'سكن خاص لا يملكه غيرك',price:7000,stock:true,stockQty:10,stockEnabled:true},
 restaurant:{id:'restaurant',name:'مطعم خاص',icon:'🍔',desc:'مطعم خاص بك داخل السيرفر',price:12000,stock:true,stockQty:5,stockEnabled:true},
 carShowroom:{id:'carShowroom',name:'معرض عربيات خاص',icon:'🏢',desc:'معرض سيارات خاص بك داخل السيرفر',price:15000,stock:true,stockQty:5,stockEnabled:true},
 villa:{id:'villa',name:'فيلا خاصة',icon:'🏡',desc:'فيلا خاصة بك داخل السيرفر',price:10000,stock:false,stockQty:0,stockEnabled:false},
 vip:{id:'vip',name:'رتبة VIP',icon:'👑',desc:'رتبة VIP بمميزات خاصة',price:3000,stock:false,stockQty:0,stockEnabled:false},
 workshop:{id:'workshop',name:'ورشة خاصة',icon:'🔧',desc:'ورشة خاصة بك داخل السيرفر',price:20000,stock:false,stockQty:0,stockEnabled:false}
};
function catalog(){ return Object.keys(DEFAULT_PRODUCTS).map(k=>{ const base=DEFAULT_PRODUCTS[k], saved=db.products[k]||{}; const p={...base,...saved}; p.stockQty=Math.max(0,Number(p.stockQty ?? (p.stock?1:0))); p.stockEnabled=p.stockEnabled!==false; p.stock=!!p.stockEnabled && p.stockQty>0; return p; }); }
...
app.post('/api/owner/products',(req,res)=>{
 ...
 db.products[id]={id,name,icon,desc,price,stockQty,stockEnabled,hiddenByCode};
 save();
...
app.post('/api/order',(req,res)=>{
 ...
 if(rawProduct.startsWith('offer:')){
   ...
   if(!product.stockEnabled || product.stockQty<=0)return res.status(400).json({error:'هذا المنتج Un stock حالياً'});
 }
 ...
 if(product){ product.stockQty=Math.max(0,product.stockQty-1); db.products[product.id]={...db.products[product.id],stockQty:product.stockQty,stockEnabled:product.stockEnabled}; }
 u.coins-=price;
 db.orders.unshift({id:crypto.randomUUID(),userId:u.id,user:u.name,email:u.email,product:name,productId:rawProduct,price,note,date:new Date().toISOString(),status:'جديد'});
 notifyUser(u.id,'تم إرسال طلب الشراء 🛒',`تم إرسال طلب ${name} بنجاح. الحالة: جديد`,'system','Falcon');
 save();
