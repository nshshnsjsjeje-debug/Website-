...
async function openSiteReviews(){
  ...
  try{
    ...
    $('siteReviewsList').innerHTML=rows.length?rows.map(r=>`<div class="reviewRow"><b>${esc(r.user)}</b><span>${'★'.repeat(Number(r.rating))}${'☆'.repeat(5-Number(r.rating))}</span><p>${esc(r.text)}</p><small>${new Date(r.date).toLocaleDateString('ar-EG')}</small></div>`).join(''):'<p class="muted">لا توجد تقييمات بعد. كن أول من يقيّم الموقع.</p>';
...
async function loadOwner(){
  try{
    const d=await api('/api/owner');
    const pd=await api('/api/owner/products'); $('productAdminList').innerHTML=(pd.products||[]).map(x=>`<div class="item"><b>${esc(x.icon)} ${esc(x.name)}</b> — ${Number(x.price).toLocaleString('ar-EG')} 🪙 — ${x.stock?'Stock: '+Number(x.stockQty||0):'Un stock'} ${x.stockEnabled===false?'🔒 مغلق':''} ${x.hiddenByCode?'— 🔒 بكود':''} <button onclick="editProductStock('${esc(x.id)}',${Number(x.stockQty||0)},${x.stockEnabled!==false})">تعديل الـStock</button> <button onclick="deleteProduct('${esc(x.id)}')">حذف</button></div>`).join('');
...
async function saveProduct(){try{const body={id:$('prodId').value.trim(),name:$('prodName').value.trim(),icon:$('prodIcon').value.trim(),desc:$('prodDesc').value.trim(),price:Number($('prodPrice').value),stockQty:Number($('prodStockQty').value||0),stockEnabled:$('prodStock').checked,hiddenByCode:$('prodHiddenByCode')?.checked};await api('/api/owner/products',{method:'POST',body});toast('تم حفظ المنتج ✓');$('prodId').value=$('prodName').value=$('prodDesc').value=$('prodPrice').value=$('prodStockQty').value='';$('prodStock').checked=false;loadOwner();}catch(e){toast('تعذر حفظ المنتج',e.message,'error')}}
