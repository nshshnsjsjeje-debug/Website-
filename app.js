...
async function pollNotifications(){
  try{
    ...
    if(Array.isArray(d.notifications)){ $('notificationCount').textContent=Number($('notificationCount').textContent||0)+d.notifications.length; }
...
function openFixedModal(id){const el=$(id);if(!el)return false;el.classList.add('open');lockPageForModal();return true;}
function closeFixedModal(id){const el=$(id);if(el)el.classList.remove('open');if(!document.querySelector('.modal.open'))unlockPageForModal();}
...
function renderSampProducts(products){$('sampProducts').innerHTML=products.map(p=>`<article class="storeItem ${p.stock?'':'out'}"><button class="favoriteBtn ${favoriteIds.has(p.id)?'active':''}" onclick="toggleFavorite('${esc(p.id)}')">${favoriteIds.has(p.id)?'♥':'♡'}</button><div class="productIcon">${esc(p.icon)}</div><h3>${esc(p.name)}</h3><p>${esc(p.desc)}</p><strong>${Number(p.price).toLocaleString('ar-EG')} 🪙</strong>${p.stock?`<button onclick="buy('${esc(p.id)}','${esc(p.name)}',${Number(p.price)})">شراء ${esc(p.name)}</button>`:'<button disabled>Un stock</button>'}</article>`).join('')||'<p class="muted">لا توجد منتجات.</p>';}
...
async function openStaffNotifications(){
  try{
    ...
    const orders=d.purchases||[], msgs=d.supportMessages||[], tickets=d.tickets||[], auctions=d.auctions||[];
    const items=[];
    orders.slice(0,12).forEach(x=>items.push(`<div class="staffNoticeItem"><b>🛒 طلب شراء جديد</b><p>${esc(x.product)} — ${esc(x.user)}</p><small>${Number(x.price).toLocaleString('ar-EG')} 🪙 • الحالة: ${esc(x.status||'جديد')}</small><div class="staffOrderActions"><button onclick="staffOrderStatus('${esc(x.id)}','قيد المراجعة')">قيد المراجعة</button><button onclick="staffOrderStatus('${esc(x.id)}','تم التسليم')">سيتم التسليم</button></div></div>`));
    msgs.slice(0,8).forEach(x=>items.push(`<div class="staffNoticeItem"><b>🎧 رسالة دعم</b><p>${esc(x.user)}: ${esc(x.message)}</p><small>${new Date(x.date).toLocaleString('ar-EG')}</small></div>`));
    tickets.slice(0,8).forEach(x=>items.push(`<div class="staffNoticeItem"><b>🎫 تذكرة دعم</b><p>${esc(x.user)} — ${esc(x.subject)}</p><small>الحالة: ${esc(x.status)}</small><button class="staffTicketBtn" onclick="openStaffTicket('${esc(x.id)}')">فتح التذكرة والرد</button></div>`));
    auctions.filter(x=>x.status==='active').forEach(x=>items.push(`<div class="staffNoticeItem"><b>🏆 مزاد مباشر</b><p>${esc(x.title)}</p><small>المزايدات: ${(x.bids||[]).length}</small></div>`));
    $('staffNotifyList').innerHTML=items.join('')||'<div class="emptyAuction">لا توجد طلبات أو رسائل جديدة حاليًا.</div>';
    $('staffNotificationCount').textContent=orders.length+msgs.length+tickets.length;
...
function openSettings(){
  ...
  api('/api/me').then(d=>{if(!d.loggedIn)return showAuth('login');$('settingsUsername').value=d.user.username||'';$('settingsOldPass').value='';$('settingsNewPass').value='';openFixedModal('settingsModal')}).catch(()=>showAuth('login'));
}
async function saveSettings(){
  try{
    const username=$('settingsUsername').value.trim(),oldPassword=$('settingsOldPass').value,newPassword=$('settingsNewPass').value;
    const d=await api('/api/settings',{method:'POST',body:{username,oldPassword,newPassword}});
...
async function loadOwner(){
  try{
    const d=await api('/api/owner');
    const pd=await api('/api/owner/products'); $('productAdminList').innerHTML=(pd.products||[]).map(x=>`<div class="item"><b>${esc(x.icon)} ${esc(x.name)}</b> — ${Number(x.price).toLocaleString('ar-EG')} 🪙 — ${x.stock?'Stock':'Un stock'} ${x.hiddenByCode?'— 🔒 بكود':''} <button onclick="deleteProduct('${esc(x.id)}')">حذف</button></div>`).join('');
...
async function saveProduct(){try{const body={id:$('prodId').value.trim(),name:$('prodName').value.trim(),icon:$('prodIcon').value.trim(),desc:$('prodDesc').value.trim(),price:Number($('prodPrice').value),stock:$('prodStock').checked,hiddenByCode:$('prodHiddenByCode')?.checked};await api('/api/owner/products',{method:'POST',body});toast('تم حفظ المنتج ✓');$('prodId').value=$('prodName').value=$('prodDesc').value=$('prodPrice').value='';loadOwner();}catch(e){toast('تعذر حفظ المنتج',e.message,'error')}}
...
async function saveMaintenance(){try{const d=await api('/api/owner/maintenance',{method:'POST',body:{enabled:$('maintenanceEnabled').checked,message:$('maintenanceMessage').value}});toast('تم تحديث وضع الصيانة ✓',d.maintenance.enabled?'الموقع الآن في وضع الصيانة.':'تم إيقاف وضع الصيانة.');loadSiteConfig(true,true);}catch(e){toast('تعذر حفظ وضع الصيانة',e.message,'error')}}
...
async function init(showDaily=true){
  try{
    ...
    await loadSiteConfig(!!d.owner,!!d.staff);
    if(d.loggedIn){
      ...
      if(d.staff){ try{ const od=await api('/api/owner'); $('staffNotificationCount').textContent=(od.purchases||[]).length+(od.supportMessages||[]).length+(od.tickets||[]).length; }catch(e){} }
