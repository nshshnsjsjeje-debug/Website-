...
async function openNotificationCenter(){try{const d=await api('/api/notifications/all');$('notificationCount').textContent=d.unread||0;$('notificationList').innerHTML=(d.notifications||[]).map(n=>`<div class="notificationItem ${n.read?'read':'unread'}"><div class="notificationIcon">${n.type==='offer'?'🔥':n.type==='gift'?'🎁':n.type==='system'?'⚙️':'🔔'}</div><div><b>${esc(n.title)}</b><p>${esc(n.text)}</p><small>${n.sender?`من ${esc(n.sender)} • `:''}${new Date(n.createdAt).toLocaleString('ar-EG')}</small></div></div>`).join('')||'<p class="muted">لا توجد إشعارات.</p>';openFixedModal('notificationCenterModal');}catch(e){if(e.status===401)showAuth('login');else toast('تعذر فتح الإشعارات',e.message,'error')}}
...
async function pollNotifications(){
  try{
    ...
    if(Array.isArray(d.notifications)){ $('notificationCount').textContent=d.notifications.length; }
...
async function openSiteReviews(){
  ...
  try{
    ...
    $('siteReviewsList').innerHTML=rows.length?rows.map(r=>`<div class="reviewRow">${r.avatarUrl?`<img src="${esc(r.avatarUrl)}" class="reviewAvatar" onerror="this.style.display='none'">`:''}<div><b>${esc(r.user)}</b><span>${'★'.repeat(Number(r.rating))}${'☆'.repeat(5-Number(r.rating))}</span><p>${esc(r.text)}</p><small>${new Date(r.date).toLocaleDateString('ar-EG')}</small></div></div>`).join(''):'<p class="muted">لا توجد تقييمات بعد. كن أول من يقيّم الموقع.</p>';
...
function buy(product,name,price){
  ...
  openModal('شراء '+name,`السعر ${Number(price).toLocaleString()} عملة فالكون.`,extra);
...
function renderSampProducts(products){$('sampProducts').innerHTML=products.map(p=>{const final=Number(p.finalPrice??p.price);const old=Number(p.originalPrice??p.price);const disc=Number(p.discountPercent||0);return `<article class="storeItem ${p.stock?'':'out'}"><button class="favoriteBtn ${favoriteIds.has(p.id)?'active':''}" onclick="toggleFavorite('${esc(p.id)}')">${favoriteIds.has(p.id)?'♥':'♡'}</button><div class="productIcon">${esc(p.icon)}</div><h3>${esc(p.name)}</h3><p>${esc(p.desc)}</p>${disc?`<div class="discountBadge">-${disc}%</div><div class="priceLine"><del>${old.toLocaleString('ar-EG')} 🪙</del><strong>${final.toLocaleString('ar-EG')} 🪙</strong></div>`:`<strong>${final.toLocaleString('ar-EG')} 🪙</strong>`}<span class="stockStatus ${p.stock?'in':'out'}">${p.stock?'🟢 متوفر':'🔴 غير متوفر'}</span>${p.stock?`<button onclick="buy('${esc(p.id)}','${esc(p.name)}',${final})">شراء ${esc(p.name)}</button>`:'<button disabled>Un stock</button>'}</article>`}).join('')||'<p class="muted">لا توجد منتجات.</p>';}
...
async function openStaffNotifications(){try{const d=await api('/api/staff-notifications');const rows=d.notifications||[];const items=[];rows.slice(0,40).forEach(n=>{let actions='';if(n.type==='order'&&n.orderId)actions=`<div class="staffOrderActions"><button onclick="staffOrderStatus('${esc(n.orderId)}','تم التسليم')">تم تسليم</button><button onclick="staffOrderStatus('${esc(n.orderId)}','ملغي')">إلغاء</button></div>`;items.push(`<div class="staffNoticeItem ${n.read?'read':''}"><b>${esc(n.title)}</b><p>${esc(n.text)}</p><small>${new Date(n.createdAt).toLocaleString('ar-EG')}</small>${actions}</div>`)});$('staffNotifyList').innerHTML=items.join('')||'<div class="emptyAuction">لا توجد إشعارات إدارية حاليًا.</div>';$('staffNotificationCount').textContent=d.unread||0;openFixedModal('staffNotifyModal');}catch(e){if(e.status===401)showAuth('login');else toast('تعذر فتح إشعارات الإدارة',e.message,'error')}}
async function readAllStaffNotifications(){try{await api('/api/staff-notifications/read-all',{method:'POST'});openStaffNotifications()}catch(e){}}
async function staffOrderStatus(id,status){try{await api('/api/owner/order-status',{method:'POST',body:{id,status}});toast('تم تحديث الطلب ✓',status);openStaffNotifications();}catch(e){toast('تعذر تحديث الطلب',e.message,'error')}}
...
function openSettings(){
  ...
  api('/api/me').then(d=>{if(!d.loggedIn)return showAuth('login');$('settingsUsername').value=d.user.username||'';$('settingsAvatarUrl').value=d.user.avatarUrl||'';$('settingsOldPass').value='';$('settingsNewPass').value='';openFixedModal('settingsModal')}).catch(()=>showAuth('login'));
}
async function saveSettings(){
  try{
    const username=$('settingsUsername').value.trim(),oldPassword=$('settingsOldPass').value,newPassword=$('settingsNewPass').value,avatarUrl=$('settingsAvatarUrl').value.trim();
    const d=await api('/api/settings',{method:'POST',body:{username,oldPassword,newPassword,avatarUrl}});
...
async function loadOwner(){
  try{
    ...
    const pd=await api('/api/owner/products'); $('productAdminList').innerHTML=(pd.products||[]).map(x=>`<div class="item"><b>${esc(x.icon)} ${esc(x.name)}</b> — ${Number(x.price).toLocaleString('ar-EG')} 🪙 — ${x.stock?'Stock':'Un stock'} ${x.discountPercent?`— خصم ${x.discountPercent}%`:''} ${x.hiddenByCode?'— 🔒 بكود':''} <button onclick="setProductDiscount('${esc(x.id)}')">تعديل الخصم</button> <button onclick="deleteProduct('${esc(x.id)}')">حذف</button></div>`).join('');
...
async function saveProduct(){try{const body={id:$('prodId').value.trim(),name:$('prodName').value.trim(),icon:$('prodIcon').value.trim(),desc:$('prodDesc').value.trim(),price:Number($('prodPrice').value),stock:$('prodStock').checked,hiddenByCode:$('prodHiddenByCode')?.checked,discountPercent:Number($('prodDiscount')?.value||0)};await api('/api/owner/products',{method:'POST',body});toast('تم حفظ المنتج ✓');$('prodId').value=$('prodName').value=$('prodDesc').value=$('prodPrice').value=$('prodDiscount').value='';loadOwner();}catch(e){toast('تعذر حفظ المنتج',e.message,'error')}}
async function setProductDiscount(id){const p=prompt('نسبة الخصم من 0 إلى 100:','0');if(p===null)return;const percent=Number(p);if(!Number.isFinite(percent)||percent<0||percent>100)return toast('قيمة الخصم غير صحيحة','اكتب نسبة بين 0 و100.','error');try{await api('/api/staff/products/'+encodeURIComponent(id)+'/discount',{method:'POST',body:{discountPercent:percent}});toast('تم تحديث الخصم ✓',percent?`خصم ${percent}%`:'تم إلغاء الخصم');loadOwner();}catch(e){toast('تعذر تعديل الخصم',e.message,'error')}}
...
async function init(showDaily=true){
  try{
    ...
    if(d.loggedIn){
      ...
      $('account').innerHTML=`<button class="accountBtn accountWithAvatar" onclick="openSettings()">${d.user.avatarUrl?`<img src="${esc(d.user.avatarUrl)}" class="avatarImg" onerror="this.style.display='none'">`:'<span class="avatarText">'+esc((d.user.username||'F')[0])+'</span>'} <span>⚙️ ${esc(d.user.username)}</span></button>`;
