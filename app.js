...
/* ===== Falcon New: Wheel / Inbox / Quick Offers / User Card ===== */
const FALCON_WHEEL = [
  {weight:40,label:'عالي جدًا — طلع موت'},
  {weight:50,label:'عالي — عادي'},
  {weight:60,label:'مش أوي'},
  {weight:80,label:'تحت المتوسط'},
  {weight:100,label:'قليل جدًا — موت'},
  {weight:150,label:'الأقل'}
];
function weightedFalconPick(){
  const total=FALCON_WHEEL.reduce((s,x)=>s+x.weight,0);
  let n=Math.random()*total;
  for(const x of FALCON_WHEEL){n-=x.weight;if(n<=0)return x;}
  return FALCON_WHEEL[FALCON_WHEEL.length-1];
}
function openWheel(){if(!openFixedModal('wheelModal'))return;$('wheelResult').textContent='اضغط لبدء العجلة';}
function spinWheel(){
  const btn=$('spinWheelBtn'), out=$('wheelResult'); if(!btn||!out)return;
  btn.disabled=true; out.textContent='🎡 جاري الدوران...';
  setTimeout(()=>{const r=weightedFalconPick();out.textContent='🎉 النتيجة: '+r.label;btn.disabled=false;},900);
}
async function openMessageInbox(){
  if(!openFixedModal('messageInboxModal'))return;
  const box=$('messageInboxList'); box.innerHTML='<p class="muted">جاري تحميل الرسائل...</p>';
  try{
    const d=await api('/api/user-messages');
    const rows=d.messages||[];
    box.innerHTML=rows.length?rows.map(m=>`<article class="inboxMsg ${m.read?'':'unread'}"><div><b>${esc(m.title)}</b><small>${new Date(m.date).toLocaleString('ar-EG')}</small></div><p>${esc(m.text)}</p>${m.read?'':'<button onclick="markMessageRead(\''+esc(m.id)+'\')">تمت القراءة</button>'}</article>`).join(''):'<p class="muted">لا توجد رسائل حاليًا.</p>';
  }catch(e){if(e.status===401){closeFixedModal('messageInboxModal');showAuth('login')}else toast('تعذر تحميل صندوق الرسائل',e.message,'error')}
}
async function markMessageRead(id){try{await api('/api/user-messages/'+encodeURIComponent(id)+'/read',{method:'POST'});openMessageInbox()}catch(e){toast('تعذر تحديث الرسالة',e.message,'error')}}
async function openUserCard(){
  if(!openFixedModal('userCardModal'))return;
  const box=$('userCardContent');box.innerHTML='<p class="muted">جاري تحميل البطاقة...</p>';
  try{
    const d=await api('/api/me'); if(!d.loggedIn){closeFixedModal('userCardModal');showAuth('login');return;}
    const u=d.user||{}, coins=Number(u.coins||0), level=Math.max(1,Math.floor(coins/5000)+1);
    box.innerHTML=`<div class="userCardVisual"><div class="userAvatar">F</div><h3>${esc(u.username||u.name||'مستخدم')}</h3><span>المستوى ${level}</span></div><div class="userCardStats"><div><b>${coins.toLocaleString('ar-EG')}</b><small>🪙 العملات</small></div><div><b>${level}</b><small>🏆 المستوى</small></div><div><b>${d.owner?'الإدارة':'عضو'}</b><small>الصفة</small></div></div>`;
  }catch(e){toast('تعذر تحميل بطاقة المستخدم',e.message,'error')}
}
async function loadQuickOffers(){
  try{
    const d=await api('/api/quick-offers'),box=$('quickOffersList');if(!box)return;
    box.innerHTML=d.offers?.length?d.offers.map(o=>`<article class="quickOffer"><div><b>${esc(o.product)}</b><small>${Number(o.price).toLocaleString('ar-EG')} 🪙 ${o.discount?`— خصم ${o.discount}%`:''}</small></div><strong>عرض سريع ⚡</strong></article>`).join(''):'<p class="muted">لا توجد عروض سريعة حاليًا.</p>';
  }catch(e){}
}
async function sendUserMessage(){
  const username=$('messageUsername').value.trim(),title=$('messageTitle').value.trim(),text=$('messageText').value.trim();
  if(!username||!title||!text)return toast('أكمل بيانات الرسالة','اسم المستخدم والعنوان والرسالة مطلوبة.','error');
  try{await api('/api/owner/user-message',{method:'POST',body:{username,title,text}});$('messageUsername').value=$('messageTitle').value=$('messageText').value='';toast('تم إرسال الرسالة ✉️');}
  catch(e){toast('تعذر إرسال الرسالة',e.message,'error')}
}
async function createQuickOffer(){
  const product=$('quickOfferProduct').value.trim(),price=Number($('quickOfferPrice').value),discount=Number($('quickOfferDiscount').value||0),minutes=Number($('quickOfferMinutes').value);
  try{await api('/api/owner/quick-offers',{method:'POST',body:{product,price,discount,minutes}});$('quickOfferProduct').value=$('quickOfferPrice').value=$('quickOfferDiscount').value=$('quickOfferMinutes').value='';toast('تم إنشاء العرض السريع ⚡');loadQuickOffers();}
  catch(e){toast('تعذر إنشاء العرض',e.message,'error')}
}
async function loadQuickOfferAdmin(){
  try{const d=await api('/api/quick-offers'),box=$('quickOfferAdminList');if(box)box.innerHTML=(d.offers||[]).map(o=>`<div class="item"><b>${esc(o.product)}</b> — ${Number(o.price).toLocaleString('ar-EG')} 🪙 — ${o.discount}% خصم <button onclick="deleteQuickOffer('${esc(o.id)}')">حذف</button></div>`).join('')||'<span class="muted">لا توجد عروض.</span>';}catch(e){}
}
async function deleteQuickOffer(id){try{await api('/api/owner/quick-offers/'+encodeURIComponent(id),{method:'DELETE'});loadQuickOfferAdmin();loadQuickOffers()}catch(e){toast('تعذر حذف العرض',e.message,'error')}}
async function loadQuickOffers(){try{const d=await api('/api/quick-offers'),box=$('quickOffersList');if(box)box.innerHTML=(d.offers||[]).map(o=>`<article class="quickOffer"><div><b>${esc(o.product)}</b><small>${Number(o.price).toLocaleString('ar-EG')} 🪙 ${o.discount?`— خصم ${o.discount}%`:''}</small></div><strong>⚡ عرض سريع</strong></article>`).join('')||'<p class="muted">لا توجد عروض سريعة حاليًا.</p>';}catch(e){}}
setTimeout(loadQuickOffers,700);
setTimeout(loadQuickOfferAdmin,1000);
