...
function toggleMenu(){
  ...
  // Menus are overlays, not full pages: keep the document scrollable on phones.
  if(!opening) unlockPageForModal();
}
...
function openFixedModal(id){
  const el=$(id);
  if(!el)return false;
  // Always render dialogs directly under <body>. This prevents any parent
  // transform/filter/overflow from changing the fixed-position reference on mobile.
  // The dialog is therefore anchored to the member's CURRENT VIEWPORT, not to
  // the product, button, section, or document scroll position.
