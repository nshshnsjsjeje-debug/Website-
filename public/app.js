...
function toast(title,text='',type='success'){
  ...
  // Toasts belong to the member's current document position, not a fixed
  // location at the top of the site.
...
function openModal(title,desc,extra='',anchor=null){
  ...
  // All ordinary dialogs are viewport-relative, never button-relative.
  // They open in the exact screen/viewport the member is currently viewing.
  // `anchor` is intentionally ignored so scrolling to a product does not
  // change where the dialog is rendered or move the member elsewhere.
  if(box){
    box.style.position='';
    box.style.top='';
    box.style.left='';
...
function toggleMenu(){
  ...
  m.setAttribute('aria-hidden',String(!opening));
  if($('menuCartCount')&&$('cartCount')) $('menuCartCount').textContent=$('cartCount').textContent||'0';
  if($('menuNotificationCount')&&$('notificationCount')) $('menuNotificationCount').textContent=$('notificationCount').textContent||'0';
  // Never lock the document for the three-dot menu: the page keeps one normal scroll.
...
function buy(product,name,price,anchor=null){
  window.location.href='/buy.html?product='+encodeURIComponent(product);
}
...
function isFullPageModal(el){ return !!(el && (el.classList.contains('pageModal') || el.querySelector('.fullPageBox'))); }
function openFixedModal(id){
  ...
  // Normal dialogs must NOT lock the document on mobile.
  // The member can keep scrolling exactly where they are; only full-page
  // sections (for example the SAMP page) use the page lock.
  if(isFullPageModal(el)) lockPageForModal(); else unlockPageForModal();
...
function closeFixedModal(id){
  ...
  if(!active || isFullPageModal(active)) {
    if(active && isFullPageModal(active)) lockPageForModal();
...
function openSettings(){
  ...
  api('/api/me').then(d=>{if(!d.loggedIn)return showAuth('login');$('settingsUsername').value=d.user.username||'';$('settingsAvatar').value=(d.user.avatar||'').startsWith('data:')?'':(d.user.avatar||'');if($('settingsAvatarPreview')){if(d.user.avatar){$('settingsAvatarPreview').src=d.user.avatar;$('settingsAvatarPreview').style.display='block'}else $('settingsAvatarPreview').style.display='none'}$('settingsOldPass').value='';$('settingsNewPass').value='';openFixedModal('settingsModal')}).catch(()=>showAuth('login'));
}
function previewAvatarFile(input){const file=input.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{const max=512,scale=Math.min(1,max/img.width,max/img.height),c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.width*scale));c.height=Math.max(1,Math.round(img.height*scale));c.getContext('2d').drawImage(img,0,0,c.width,c.height);const data=c.toDataURL('image/jpeg',.78);$('settingsAvatar').value=data;const p=$('settingsAvatarPreview');if(p){p.src=data;p.style.display='block'}};img.src=reader.result};reader.readAsDataURL(file)}
...
function updateCoins(n){const v=Number(n||0).toLocaleString('ar-EG');if($('coinsTop'))$('coinsTop').textContent=v;if($('heroCoins'))$('heroCoins').textContent=v}
function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}

function previewProductImage(input){const file=input.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{const max=512,scale=Math.min(1,max/img.width,max/img.height),c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.width*scale));c.height=Math.max(1,Math.round(img.height*scale));c.getContext('2d').drawImage(img,0,0,c.width,c.height);input.dataset.data=c.toDataURL('image/jpeg',.78);};img.src=reader.result};reader.readAsDataURL(file)}
function previewProductImage(input){const file=input.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{const max=512,scale=Math.min(1,max/img.width,max/img.height),c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.width*scale));c.height=Math.max(1,Math.round(img.height*scale));c.getContext('2d').drawImage(img,0,0,c.width,c.height);input.dataset.data=c.toDataURL('image/jpeg',.78);};img.src=reader.result};reader.readAsDataURL(file)}
async function saveProduct(){try{const body={id:$('prodId').value.trim(),name:$('prodName').value.trim(),icon:$('prodIcon').value.trim(),desc:$('prodDesc').value.trim(),price:Number($('prodPrice').value),discountPercent:Number($('prodDiscount').value||0),stockQty:Number($('prodStockQty').value||0),stockEnabled:$('prodStock').checked,hiddenByCode:$('prodHiddenByCode')?.checked,image:$('prodImage')?.dataset.data||''};await api('/api/owner/products',{method:'POST',body});toast('تم حفظ المنتج ✓');['prodId','prodName','prodDesc','prodPrice','prodStockQty','prodDiscount','prodImage'].forEach(id=>{if($(id))$(id).value=''});$('prodStock').checked=false;loadOwner();}catch(e){toast('تعذر حفظ المنتج',e.message,'error')}}
...
async function giveCoinsToUser(){const username=$('giveCoinsUser')?.value.trim(),amount=Number($('giveCoinsAmount')?.value||0);if(!username||amount<1)return toast('أكمل البيانات','اكتب اسم العضو وعدد العملات.','error');try{const d=await api('/api/owner/give-coins',{method:'POST',body:{username,amount}});$('giveCoinsUser').value=$('giveCoinsAmount').value='';toast('تم إرسال العملات ✈️',`رصيد العضو الآن ${Number(d.coins).toLocaleString('ar-EG')}`)}catch(e){toast('تعذر إرسال العملات',e.message,'error')}}
async function giveCoinsToUser(){const username=$('giveCoinsUser')?.value.trim(),amount=Number($('giveCoinsAmount')?.value||0);if(!username||amount<1)return toast('أكمل البيانات','اكتب اسم العضو وعدد العملات.','error');try{const d=await api('/api/owner/give-coins',{method:'POST',body:{username,amount}});$('giveCoinsUser').value=$('giveCoinsAmount').value='';toast('تم إرسال العملات ✈️',`رصيد العضو الآن ${Number(d.coins).toLocaleString('ar-EG')}`)}catch(e){toast('تعذر إرسال العملات',e.message,'error')}}
...
async function init(showDaily=true){
  try{
    ...
    if(d.loggedIn){
      ...
      if($('menuStaffArea')) $('menuStaffArea').style.display=d.staff?'grid':'none';
      $('owner').style.display=d.staff?'block':'none';
      if($('staffNotify')) $('staffNotify').style.display=d.staff?'inline-flex':'none';
      if(d.staff){ try{ const sn=await api('/api/staff/notifications'); if($('staffNotificationCount')) $('staffNotificationCount').textContent=sn.unread||0; if($('menuStaffNotificationCount')) $('menuStaffNotificationCount').textContent=sn.unread||0; }catch(e){} }
      ...
      if($('menuStaffArea')) $('menuStaffArea').style.display='none'; if($('owner')) $('owner').style.display='none'; if($('staffNotify')) $('staffNotify').style.display='none'; document.querySelectorAll('.ownerOnlyCard').forEach(el=>el.style.display='none'); clearInterval(window.__liveDashTimer);
    }
...
window.addEventListener('load',async()=>{
  ...
  if(params.get('purchase')==='success'){
    ...
    setTimeout(()=>toast('تم الشراء بنجاح ✓',`تم تسجيل طلب ${productName} وإرساله لصاحب الموقع/الإدارة.`),500);
...
/* Falcon global overlay normalizer: applies to every dynamically opened dialog/popover. */
(function(){
  const selectors = [
    '.modal','.modal-overlay','.popup','.popover','.dialog-overlay',
    '.confirm-modal','.confirmation-modal'
  ];
  function normalize(root=document){
    root.querySelectorAll(selectors.join(',')).forEach(el=>{
      if (!el.classList.contains('falcon-viewport-overlay')) {
        el.classList.add('falcon-viewport-overlay');
      }
    });
  }
  normalize();
  new MutationObserver(muts=>{
    for(const m of muts){
      m.addedNodes.forEach(n=>{
        if(n.nodeType===1){
          if(selectors.some(s=>n.matches?.(s))) n.classList.add('falcon-viewport-overlay');
          normalize(n);
        }
      });
    }
...
/* === FINAL RESPONSIVE VIEWPORT CALIBRATION === */
(function(){
  function syncFalconViewport(){
    const vv=window.visualViewport;
    const h=vv ? vv.height : window.innerHeight;
    document.documentElement.style.setProperty('--falcon-vh',Math.max(1,h)+'px');
  }
  syncFalconViewport();
  window.addEventListener('resize',syncFalconViewport,{passive:true});
  window.addEventListener('orientationchange',()=>setTimeout(syncFalconViewport,80),{passive:true});
  if(window.visualViewport){
    window.visualViewport.addEventListener('resize',syncFalconViewport,{passive:true});
  }
})();
