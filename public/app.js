...
async function giftSpecificUser(){
  const username=$('giftUserName')?.value.trim(),amount=Math.floor(Number($('giftUserAmount')?.value||0));
  if(!username||!amount||amount<1)return toast('أكمل البيانات','اكتب اسم العضو وعدد العملات.','error');
  try{const d=await api('/api/owner/gift-user',{method:'POST',body:{username,amount}});$('giftUserName').value='';$('giftUserAmount').value='';toast('تم إرسال العملات ✈️',`تمت إضافة ${amount.toLocaleString('ar-EG')} عملة إلى ${d.user.username}.`);loadOwner();}
  catch(e){toast('تعذر إرسال العملات',e.message,'error')}
}
...
function openSettings(){
  ...
  api('/api/me').then(d=>{if(!d.loggedIn)return showAuth('login');$('settingsUsername').value=d.user.username||'';$('settingsAvatar').value=(d.user.avatar&&String(d.user.avatar).startsWith('data:image/'))?'':(d.user.avatar||'');$('settingsOldPass').value='';$('settingsNewPass').value='';if($('settingsAvatarFile'))$('settingsAvatarFile').value='';openFixedModal('settingsModal')}).catch(()=>showAuth('login'));
}
async function saveSettings(){
  try{
    ...
    let avatarData=''; const file=$('settingsAvatarFile')?.files?.[0];
    if(file){
      if(file.size>1500000) return toast('الصورة كبيرة','اختار صورة أقل من 1.5MB.','error');
      avatarData=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result||''));r.onerror=reject;r.readAsDataURL(file);});
    }
    const d=await api('/api/settings',{method:'POST',body:{username,avatar,avatarData,oldPassword,newPassword}});
...
async function init(showDaily=true){
  try{
    ...
    if(d.loggedIn){
      ...
      $('account').innerHTML=`<button class="accountBtn" onclick="openSettings()">${d.user.avatar?`<img class="accountAvatarMini" src="${esc(d.user.avatar)}" alt="">`:''} ⚙️ ${esc(d.user.username)}</button>`;
      if($('menuStaffArea')) $('menuStaffArea').style.display=d.staff?'grid':'none'; if($('staffBottomLink')) $('staffBottomLink').style.display=d.staff?'block':'none';
      ...
      if($('menuStaffArea')) $('menuStaffArea').style.display='none'; if($('staffBottomLink')) $('staffBottomLink').style.display='none'; if($('owner')) $('owner').style.display='none'; if($('staffNotify')) $('staffNotify').style.display='none'; document.querySelectorAll('.ownerOnlyCard').forEach(el=>el.style.display='none'); clearInterval(window.__liveDashTimer);
    }
